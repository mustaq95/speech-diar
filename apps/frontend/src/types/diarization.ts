/**
 * The Unified Diarization Contract.
 *
 * Every backend engine (PyAnnote, WhisperX, NeMo, cloud APIs, ...) emits its
 * own native JSON. Nothing under `src/components/` may depend on those native
 * shapes: an adapter in `src/adapters/` must translate each of them into the
 * types in this file, which are the single format the UI renders.
 *
 * This is the frontend's working copy of the platform-wide contract in
 * `packages/shared_contracts/types.ts` (mirrored by `schemas.py` for the
 * Python services). If the contract changes, keep all copies in sync.
 */
