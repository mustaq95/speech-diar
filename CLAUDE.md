...
- **`hamsa-tts-new` is the SAME vendor as `hamsa-tts`, reached through the gateway instead of the pod, and the vendor's guide for it is wrong in six places.** Probed 2026-09-09, and the probe wins every time:
  - **The documented model id `hamsa-tts-new` is a 403** (`team not allowed to access model`) on our key, which carries the model as `hamsa-tts`. `HAMSA_TTS_NEW_MODEL` exists so that flip is an `.env` edit, not a code change.
  ...
  All 113 bundled speakers were swept and every one returned real audio, so the shipped dropdown cannot produce that 500. The pod accepts the same voices, so `HAMSA_TTS_SPEAKER=Ruba,Sandra` is an under-populated `.env` list rather than a model limit — the two engines are two pipelines, not two voice sets.
