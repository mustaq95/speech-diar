...
The Transcript surface has two sub-modes, chosen by the STT/TTS toggle in the page head: STT measures speech in (read a script aloud, score engines against it), TTS measures speech out (hand the identical script to every TTS engine and compare the audio they return). Lives in `apps/background_worker/tts/` (engines), `apps/backend_api/routers/tts.py` (the routes) and `apps/frontend/src/components/TtsStudio.tsx` (the UI). Three engines now: `hamsa-tts` (the pod, `stream`), `hamsa-tts-new` (the same vendor via the gateway, `single`) and `inception-tts` (`single`).
...
- **`hamsa-tts-new` is the SAME vendor as `hamsa-tts`, reached through the gateway instead of the pod, and the vendor's guide for it is wrong in six places.** Probed 2026-09-09, and the probe wins every time:
  - **The model id IS `hamsa-tts-new`; reaching it is a question of which KEY is in `.env`.** The key this repo shipped with answered 403 `team not allowed to access model` for that id and listed 10 models, which reads as "wrong id" and is not. The key now in `.env` lists 14 including this one and is a strict superset (loses no model, and `inception-stt`/`inception-tts` behave identically on both). `hamsa-tts` is the older deployment and still resolves, but the cloned voices exist **only** on `hamsa-tts-new`. A 403 here, or a wave of voice 500s, means the credential regressed rather than the model moving.
  - **`Content-Type` is always `audio/mpeg`** here too, over a real RIFF/WAVE body. Same gateway, same lie as Inception-TTS.
  - **`response_format` is ignored**: asking for `mp3` returns real WAV. There is deliberately no setting for it, because a setting that cannot change the outcome is worse than none.
  - **It does not meaningfully stream**, so it is registered `delivery="single"` despite being read with `client.stream()`. First byte lands at 97-99% of total across three input sizes (ratios 0.973 / 0.979 / 0.989), even for a 166-second clip. Its `first_audio_ms` ≈ `synth_ms` is the correct figure, not a bug. RTF ~0.22.
  - **Its RIFF and data size fields are `0xFFFFFFFF`.** ffprobe reads these bytes correctly; Python's `wave` reports 2147483647 frames = 134217 seconds, so `probe_audio`'s `wave` fallback treats a sentinel frame count as unknown (`_WAVE_SIZE_SENTINEL`) rather than reporting a 37-hour duration and the fabricated RTF that follows. Unlike the pod, this engine's sample rate is **measured** off its own container, which is why nothing labels it "(assumed)".
  - **An unknown voice is a 500, not the documented 400**, and empty input is a **200 carrying a 44-byte header-only WAV**, not a validation error. The adapter rejects a frameless body rather than store a clip whose duration renders as 0.
  All 113 bundled speakers were swept and every one returned real audio, so no BUILT-IN voice can produce that 500.
  - **The 12 cloned `adeo_*` voices work, and only on `hamsa-tts-new`.** Swept 2026-09-09: 125/125 voices (113 bundled + 12 cloned) returned real WAV in 31s. On `hamsa-tts` every clone answers 500 while built-ins on the same call succeed, and the direct pod drops the stream for them, so a clone failure points at `HAMSA_TTS_NEW_MODEL` or the key, never at the name (`adeo_Mariam`, `ADEO_mariam`, `adeo-mariam` all 500 even on the right model; bare `mariam` resolves to the built-in of that name). They can still vanish: clone reference audio lives in the pod's **ephemeral `/tmp`**, so a pod restart de-registers every clone and the clone flow must be re-run, and `test_live_tts.py::test_hamsa_new_cloned_voices_are_registered` is the only thing that reports it. Clear `HAMSA_TTS_NEW_CLONED_VOICE` to stop offering them. The `adeo_` prefix is load-bearing: 8 of the 12 collide by name with a bundled speaker (`Mariam`, `Reem`, `Leo`, `Emma`, `Lily`, `Claire`, `Henry`, `Mia`), and two identically-rendered dropdown rows would make the operator's pick, the stored clip's S3 key and the scorecard's voice column ambiguous. The vendor's guide also lists `Faisal` as a built-in; it is not in the 113 and it 500s.
  - **`Ali` returns near-silence for short Arabic input.** 200 with a 684-byte body (0.02s of PCM) for a 1-to-5-word Arabic phrase, reproducibly, while a 9-word phrase renders a normal 3.3s; `Zeina` handles the identical inputs fine. Deliberately **not** filtered: a duration floor is exactly the heuristic this repo keeps out of the audio path, and the 0.02s clip is what the engine produced. It surfaces as a 0:00 clip with a large RTF, which is the honest reading of a failed render.
  - **Cloned voices are a SEPARATE `.env` list from the built-ins, and the badge reads off that same list.** One source, so the badge can never name a voice the dropdown does not offer (`test_new_voices_are_a_subset_of_the_dropdown`). They sit at positions 1..12 of the combined list -- discoverable without scrolling 113 names -- but **never at position 0**, because that position is the default voice and a clone can vanish on a pod restart, which would make every no-voice-specified request fail. The pod accepts the same voices, so `HAMSA_TTS_SPEAKER=Ruba,Sandra` is an under-populated `.env` list rather than a model limit — the two engines are two pipelines, not two voice sets.

**Voice lists are ONE comma-separated setting per engine, entry 0 is the default** (`HAMSA_TTS_SPEAKER`, `HAMSA_TTS_NEW_VOICE`, `INCEPTION_TTS_VOICE`). There was briefly a default field beside a separate list field, and putting the list on the singular field posted `"Ruba,Sandra"` as one speaker name: the pod accepts that with a 200 and then kills the stream, which reads as a network fault rather than a config typo. `_split_csv` strips whitespace and empty entries, and `tests/test_tts_registration.py` asserts the default never contains a comma. No gateway here has a list-voices endpoint, so the dropdown is exactly what `.env` holds and nothing is discovered at runtime. `hamsa-tts-new`'s 113 names therefore live as the code default and its `.env` key stays commented out: setting it EMPTY is not the same as omitting it, because an empty value overrides the list with a blank voice the gateway answers with a 500.
